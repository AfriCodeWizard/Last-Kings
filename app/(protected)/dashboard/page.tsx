  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium font-sans">Receiving Queue</CardTitle>
            <Package className="h-4 w-4 text-gold" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold font-sans">{receivingQueue?.length || 0}</div>
            <p className="text-xs text-muted-foreground font-sans">Pending receiving sessions</p>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="font-sans">Receiving Queue</CardTitle>
            <CardDescription className="font-sans">Pending receiving sessions</CardDescription>
          </CardHeader>
          <CardContent>
            {receivingQueue && receivingQueue.length > 0 ? (
              <div className="space-y-2">
                {receivingQueue.map((session: { id: string; created_at: string }) => (
                  <div key={session.id} className="flex justify-between items-center p-2 rounded border border-gold/10">
                    <span className="text-sm font-sans">Session {session.id.slice(0, 8)}</span>
              ...
              <p className="text-sm text-muted-foreground font-sans">No pending receiving sessions</p>
            )}
          </CardContent>
        </Card>
      </div>
